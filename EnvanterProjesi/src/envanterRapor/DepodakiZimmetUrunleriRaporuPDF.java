...
public class DepodakiZimmetUrunleriRaporuPDF {

    public DepodakiZimmetUrunleriRaporuPDF() throws Exception {
        Map<String, Integer> veriler = new HashMap<String, Integer>();
        String personelAdi="";
        SimpleDateFormat baslikformat=new SimpleDateFormat("dd-MM-yyyy");
        Date tarih=new Date();
        Document document = new Document();
        PdfWriter.getInstance(document, new FileOutputStream("DepodakiZimmetUrunleriRaporu("+baslikformat.format(tarih)+").pdf"));
        document.open();
        ...
        document.add(new Paragraph("---------  Depodaki Zimmet Ürünleri Raporu ("+tarihformat.format(tarih)+")"+" ---------", FontFactory.getFont(FontFactory.TIMES_ROMAN, "ISO-8859-9", FontFactory.defaultEmbedding,13, 1, BaseColor.BLACK)));
    ...
    public static void main(String[] args) {
        try {
            DepodakiZimmetUrunleriRaporuPDF pdfTable = new DepodakiZimmetUrunleriRaporuPDF();
        } catch (Exception e) {
