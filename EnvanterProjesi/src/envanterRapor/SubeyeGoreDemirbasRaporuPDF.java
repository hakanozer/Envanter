...
public class SubeyeGoreDemirbasRaporuPDF {

    public SubeyeGoreDemirbasRaporuPDF() throws Exception {
        String subeAdi="";
        SimpleDateFormat baslikformat=new SimpleDateFormat("dd-MM-yyyy");
        Date tarih=new Date();
        Document document = new Document();
        PdfWriter.getInstance(document, new FileOutputStream("subeyeGoreDemirbasRaporu("+baslikformat.format(tarih)+").pdf"));
        document.open();
        ...
        DB db = new DB();
        try {
            ResultSet rs = db.baglan().executeQuery("select urunler.urun_adi,demirbaslar.demirbas_urun_adet,ilceler.ilce_adi,subeler.sube_adi from demirbaslar \n"
                    + "LEFT JOIN urunler on demirbaslar.demirbas_urun_id=urunler.urun_id \n"
                    + "LEFT JOIN subeler on subeler.sube_id=demirbaslar.demirbas_sube_id \n"
            ...
            while (rs.next()) {
                ...
                table.addCell(new Paragraph(rs.getString("demirbaslar.demirbas_urun_adet"), FontFactory.getFont(FontFactory.TIMES_ROMAN, "ISO-8859-9", FontFactory.defaultEmbedding,10, 1, BaseColor.BLACK)));
        ...
        document.add(new Paragraph("--------- "+subeAdi+" - Demirbaş Raporu ("+tarihformat.format(tarih)+")"+" ---------", FontFactory.getFont(FontFactory.TIMES_ROMAN, "ISO-8859-9", FontFactory.defaultEmbedding,13, 1, BaseColor.BLACK)));
    ...
    public static void main(String[] args) {
        try {
            SubeyeGoreDemirbasRaporuPDF pdfTable = new SubeyeGoreDemirbasRaporuPDF();
        } catch (Exception e) {
