...
public class personeleGoreZimmetRaporuPDF {

    public personeleGoreZimmetRaporuPDF() throws Exception {
        Map<String, Integer> veriler = new HashMap<String, Integer>();
        String personelAdi = "";
        SimpleDateFormat baslikformat = new SimpleDateFormat("dd-MM-yyyy");
        Date tarih = new Date();
        Document document = new Document();
        PdfWriter.getInstance(document, new FileOutputStream("personeleGoreZimmetRaporu(" + baslikformat.format(tarih) + ").pdf"));
        document.open();
        ...
        DB db = new DB();
        try {
            ResultSet rs = db.baglan().executeQuery("select urunler.urun_adi,zimmetler.zim_urun_adet,personeller.personel_adi,personeller.personel_soyadi from zimmetler LEFT JOIN urunler on urunler.urun_id=zimmetler.zim_urun_id LEFT JOIN personeller on personeller.personel_id=zimmetler.zim_personel_id where zimmetler.zim_personel_id='" + AdminRapor.personelRaporunaGorePersonelID + "'");
            while (rs.next()) {
                ...
                personelAdi = rs.getString("personeller.personel_adi") + " " + rs.getString("personeller.personel_soyadi");
        ...
        document.add(new Paragraph("--------- " + personelAdi + " - Zimmet Raporu (" + tarihformat.format(tarih) + ")" + " ---------", FontFactory.getFont(FontFactory.TIMES_ROMAN, "ISO-8859-9", FontFactory.defaultEmbedding, 13, 1, BaseColor.BLACK)));
    ...
    public static void main(String[] args) {
        try {
            personeleGoreZimmetRaporuPDF pdfTable = new personeleGoreZimmetRaporuPDF();
        } catch (Exception e) {
